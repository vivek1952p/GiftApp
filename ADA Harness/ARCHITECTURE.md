...
- **Evidence Analysis** (the Accessibility Rule Engine + four specialized engines)
...
The orchestrator [`analyze.ts`](scripts/analyze.ts) runs all nine stages in order —
nothing skips a stage, though stages 4–7 (the specialized engines) are individually
optional and degrade gracefully if one fails, so a single engine's failure never blocks
the report.
...
flowchart TD
    ...
    A --> P2["Stage 2 · uia-scan.ts<br/>headed browser + uia_capture.py"]
    P2 --> W4[uia-tree.json]

    A --> P3["Stage 3 · uia-rule-engine.ts<br/>UIA + DOM + AX-tree + ARIA-pattern rules"]
    W1 --> P3
    W3 --> P3
    W4 --> P3
    W5 --> P3
    P3 --> WF[uia-findings.json]

    A --> P4["Stage 4 · expected-focus-engine.ts"]
    W3 --> P4
    W6 --> P4
    P4 --> EFR[expected-focus-report.json]

    A --> P5["Stage 5 · widget-behavior-engine.ts"]
    P5 --> WBR[widget-behavior-report.json]

    A --> P6["Stage 6 · focus-management-engine.ts"]
    P6 --> FMR[focus-management-report.json]

    A --> P7["Stage 7 · interaction-prediction-engine.ts"]
    P7 --> IPR[interaction-report.json]

    A --> P8["Stage 8 · generate-summary.ts"]
    W1 --> P8 --> W2[summary.json]

    A --> P9["Stage 9 · merge-report.ts"]
    W2 --> P9
    W3 --> P9
    W4 --> P9
    W5 --> P9
...
| 2 · Windows UI Automation | `uia-scan.ts` → `uia_capture.py` | `uia-tree.json` |
| 3 · Accessibility Rule Engine | `uia-rule-engine.ts` (`accessibility-rule-engine/`) | `uia-findings.json` |
| 4 · Expected Focus Engine | `expected-focus-engine.ts` | `expected-focus-report.json` |
| 5 · Widget Behavior Engine | `widget-behavior-engine.ts` | `widget-behavior-report.json` |
| 6 · Focus Management Engine | `focus-management-engine.ts` | `focus-management-report.json` |
| 7 · Interaction Prediction Engine | `interaction-prediction-engine.ts` | `interaction-report.json` |
| 8 · Summarize | `generate-summary.ts` | `summary.json` |
| 9 · Correlate | `merge-report.ts` | `merged-report.json` |
| Dashboard | `dashboard.ts` | `dashboard.md` |

Stages 5–7 (and the UIA capture in stage 2) share one navigation helper,
...
## 3. The Accessibility Rule Engine (Stage 3)
...
```mermaid
flowchart TD
    START([npm run ada]) --> SCAN1[Scan #1<br/>analyze.ts — all 9 stages]
    ...
    subgraph ADA2["scan && compare && dashboard"]
        SCAN2 --> S9["all 9 scan stages<br/>(summary.previous.json + merged-report.previous.json<br/>snapshotted first, for an all-scanner diff)"]
...
sequenceDiagram
    ...
    NPM->>FS: comparison.md (diffed against pre-fix snapshot) + dashboard.md
...
flowchart LR
    subgraph NOAI["Deterministic (no AI)"]
        SCAN["Evidence collection + analysis<br/>(9 stages)"]
